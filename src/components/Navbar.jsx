    ...
    return <nav className={cn("fixed w-full z-40 transition-all duration-300",
        ...
        <div className="container flex items-center justify-between">
            <a className="text-3xl font-bold text-primary flex items-center"
                ...
                <span className="relative z-10 ">
                    <span className="text-glow text-foreground"> Vy Dinh</span>
                </span>
            </a>
        ...
        <div className="hidden md:flex space-x-8">
            {navItems.map((item, key) => (
                <a key ={key} href={item.href} className="text-foreground/80 hover:text-primary text-xl transition-colors duration-300">
                    {item.name}
                </a>
            ))}
        </div>
